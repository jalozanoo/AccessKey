...
class AccessKeyGUI:
    ...
    def show_setup_master_password(self):
        ...
        tk.Label(self.right_frame, text="Crear contraseña maestra", font=self.LABEL_FONT_TITLE,
        ...
        tk.Button(self.right_frame, text="Crear", width=15, bg=self.BTN_COLOR, fg="white",
                  command=lambda: self._create_master(password_entry, confirm_entry)).pack(pady=10)
    ...
    def show_login(self):
        ...
        while True:
            ...
            try:
                ...
                self.vault = Vault.from_bytes(self.crypto.decrypt(encrypted))
                self.salt = salt
                self._enable_left_buttons()
                return True
            except InvalidToken:
    ...
    def show_change_master_password(self):
        self.clear_right_panel()
        tk.Label(self.right_frame, text="Cambiar contraseña maestra", font=self.LABEL_FONT_TITLE, bg=self.BG_RIGHT).pack(pady=10)
        tk.Label(self.right_frame, text="Nueva contraseña:", bg=self.BG_RIGHT).pack(anchor="w")
        new_entry = tk.Entry(self.right_frame, show="*", width=30)
        new_entry.pack(pady=2)
        tk.Label(self.right_frame, text="Confirmar contraseña:", bg=self.BG_RIGHT).pack(anchor="w")
        confirm_entry = tk.Entry(self.right_frame, show="*", width=30)
        confirm_entry.pack(pady=2)
        tk.Button(self.right_frame, text="Cambiar", bg=self.BTN_COLOR, fg="white",
                  command=lambda: self._change_master_password(new_entry, confirm_entry)).pack(pady=10)

    def _change_master_password(self, new_entry, confirm_entry):
        ...
        if not new_pass or new_pass != confirm:
            ...
            return
        import os
        self.crypto = CryptoManager(new_pass, os.urandom(16))
        self.salt = self.crypto.key[:16]
        messagebox.showinfo("Éxito", "Contraseña maestra cambiada")
        self.show_empty_panel()
    ...
    def delete_access(self):
        ...
        if pw is None:
            return
        try:
            crypto_test = CryptoManager(pw, self.salt)
            crypto_test.decrypt(self.crypto.encrypt(self.vault.to_bytes()))
        except:
